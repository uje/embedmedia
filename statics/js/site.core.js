...
(function (window, $, undefined) {
    ...
    jm.browser = {
        isMobile: function () {
            var re = /(mobile|android|iphone|midp-2.0|midp-1.0)/ig;
    ...
    jm.models = {
        ...
        , getLink: function (links, name) {
            for (var i = 0, l = links.length; i < l; i++) {
                var link = links[i];
                if (link.title.toLowerCase().indexOf(name.toLowerCase()) != -1 || toPinyin(link.title).toLowerCase().indexOf(name.toLowerCase()) != -1) {
                    return link;
                }
            }
        }
        //重组数据
     , sortData: function (links) {
         var data = [];
         var tags = this.getTags(links);
         ...
         return data.sort(function (x, y) {
             return x.links.length < y.links.length ? 1 : -1;
         });
         //return data;
     }
    };
    jm.view = {
        ...
        , showResult: function (r) {
            var c = $('<div id="links" class="cf">');
            $.tmpl(this.getTmplHtml(this.linkTmpl), jm.models.sortData(r)).appendTo(c);

            $(this.loading).replaceWith(c);
            window.initialize && window.initialize(r);
        }
        , resizeMark: function () {
            //window.console && console.log && console.log(1);

            $(this.links).masonry({
                itemSelector: '#links .box'
	            , columnWidth: $(this.links).width() / 3
                //, isFitWidth: true
            });
            //$(this.linkMark).css({ height: 0, top: 0 });
            //var fixHeight = jm.browser.isMobile() ? 0 : $(this.searchArea).outerHeight();

            //$(this.linkMark).css({
            //    height: ($('html')[0].scrollHeight - $('.header').outerHeight() - $(".footer").outerHeight() - fixHeight) + 'px'
            //    , top: fixHeight
            //});

        }
        , search: function () {
            var k = $(this).parents(".search").find(".keyword");
            var keyword = $.trim(k.val());
            if (keyword == "") {
                k.focus();
                return;
            }
            var link = jm.models.getLink(keyword);
            if (link) {
        ...
        , initialize: function () {
            ...
            $(".search-area .keyword").focus();
            $(".btnSearch").live("click", this.search);
            $(window).bind('resize', function () {
                that.resizeMark();
            });
            jm.models.getLinks(function (data) {
                jm.view.showResult(data);
            });
        }
    };
...
function initialize(data) {
    ...
    $(".keyword").autocomplete(data, {
        ...
        formatItem: function (row, i, max) {
            return '<p class="s-link-url"><a href="' + row.url + '" target="_blank">' + row.title + '</a></p>' +
                   '<p class="s-link-mark"><span class="s-link-mark">' + row.tag + '</span></p>';
        },
        formatResult: function (row, i, max) {
            return row.title;
        },
        formatMatch: function (row, i, max) {
            return [row.title, row.url, row.tag, toPinyin(row.title)].join(" ");
        }
    ...
    var tags = $.merge(["全部"], jm.models.getTags(data));
    $.tmpl('<a href="javascript:;" class="tag-item">${$data}</a>', tags).appendTo("#tagCloud");
    $(".tag-item").bind("click", function () {
        ...
        var curBox = $('.box[data-tag="' + tag + '"]');
        $(".box").removeClass("focus");
        curBox.addClass("focus");
        jm.view.resizeMark();
    });
    $(".tag-item:eq(0)").click();
}
